...
import { Play, X, Copy, CopyPlus, Pencil, User, Home, Calendar, AlertTriangle, ArrowRight, Layout as LayoutIcon, MessageSquare, ChevronLeft } from 'lucide-react';
...
export const ClientView: React.FC<{ customerId: string; token?: string; onNavigate?: (page: string, params?: any) => void }> = ({ customerId, token, onNavigate }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        const startDate = toVnZeroHour(data.customer.start_date);
        const allowedDay = data.customer.allowed_day || getDiffDays(startDate, today) + 1;
        if (allowedDay > 10) {
          setIsExpanded(true);
        }
        
        if (data.customer.expire_warning === true) {
          setTimeout(() => {
            setInfoModal({ 
              isOpen: true, 
              title: "Nhắc nhở gia hạn", 
              message: `Phác đồ cá nhân của bạn sắp kết thúc thời hạn vào ngày ${formatDDMMYYYY(data.customer.end_date)}. Hãy gia hạn sớm để không gián đoạn việc tập luyện nhé!`, 
              type: "WARNING",
              color: "#F97316"
            });
          }, 1000);
  ...
  const processTaskSelection = (task: ExerciseTask) => {
    // Chỉ chạy cập nhật dữ liệu khi bài tập đang hoạt động và có ngày chia hết cho 3
    if (task.day % 3 === 0) {
      ...
      <div className={`${showAdminUI ? 'pt-28' : 'pt-10'} max-w-[1200px] mx-auto px-4 pb-20`}>
        <header className="text-center mb-12">
          ...
          <div className="inline-flex items-center gap-2 bg-white px-6 py-2.5 rounded-full border border-blue-100 text-[#2563EB] font-bold text-xs uppercase tracking-widest shadow-sm">
            <Calendar size={14} /> BẮT ĐẦU: {formatDDMMYYYY(customer.start_date)}
          </div>
        </header>
        ...
        <section className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-sm border border-blue-100 mb-10">
           ...
           <div className="text-[#1E3A8A] leading-relaxed text-base md:text-lg font-medium whitespace-pre-line text-justify">{customer.note || "Hệ thống đang cập nhật nội dung..."}</div>
        </section>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="lg:w-80 flex flex-col gap-6 flex-shrink-0">
             <div className="bg-[#1E3A8A] text-white rounded-[2.5rem] p-8 shadow-xl">
                ...
                <p className="text-sm leading-relaxed opacity-90 text-justify whitespace-pre-line font-medium">{customer.chewing_status || "Đang cập nhật chỉ dẫn ăn nhai..."}</p>
             </div>
             
             {sidebarBlocks.map((block: any) => (
               <div key={block.id} className={`rounded-[2.5rem] p-8 border transition-all ${block.type === 'dark' ? 'bg-[#1E3A8A] text-white border-transparent' : 'bg-white border-blue-50 shadow-sm'}`}>
                 <h3 className="text-lg font-black mb-4 border-b border-opacity-10 pb-3 flex items-center gap-2 uppercase tracking-wide">{block.title}</h3>
                 <p className="text-sm leading-relaxed mb-6 whitespace-pre-line font-medium text-justify">{block.content}</p>
                 ...
                 {block.is_chat ? (
                   ...
                   <button onClick={() => window.open(block.video_link || block.videoLink, '_blank')} className={`w-full py-4 rounded-full font-black text-[10px] uppercase flex items-center justify-center gap-2 shadow-lg transition-all hover:opacity-90 active:scale-95 ${block.type === 'dark' ? 'bg-white text-blue-900' : 'bg-blue-600 text-white'}`}>▶ Xem hướng dẫn</button>
                 )}
               </div>
             ))}
          </aside>
      ...
      {selectedTask && (
        <div className="fixed inset-0 z-[6000] flex items-center justify-center p-4 animate-in fade-in duration-200">
          ...
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] relative z-10 overflow-hidden shadow-2xl animate-in zoom-in duration-300">
             ...
             <div className="p-8 pt-0">{selectedTask.link && <button onClick={() => window.open(selectedTask.link, '_blank')} className="w-full py-4 bg-blue-600 text-white font-bold rounded-full shadow-lg flex items-center justify-center gap-2 uppercase text-xs tracking-widest transition-all hover:bg-blue-700 active:scale-95">▶ Xem hướng dẫn bài tập</button>}</div>
          </div>
        </div>
      )}
