...
import { Search, Plus, Trash2, Calendar, CheckSquare, Square, Loader2, ChevronDown, Target, CheckCircle2, Zap, Users, RefreshCw, AlertCircle, Play, FileSpreadsheet, Edit3, Eye, Home } from 'lucide-react';
...
export const VideoGroupManagement: React.FC<{ onNavigate: (page: string, params?: any) => void }> = ({ onNavigate }) => {
  ...
  const fetchTasksForSelected = useCallback(async (keys: string[]) => {
    ...
    try {
      ...
      setTasks(all.sort((a, b) => (Number(a.day) || 0) - (Number(b.day) || 0)));
    } catch (e) { 
  ...
  const handleConfirmedDelete = async () => {
    ...
    if (type === 'task') {
      setTasksLoading(true);
      try {
        await api.deleteVideoTask(payload._rowNumber!);
    ...
    <MainLayout title="QUẢN LÝ PHÁC ĐỒ MẪU" onBack={() => onNavigate('dashboard')} actions={
      ...
      <Modal isOpen={isNewGroupModalOpen} onClose={() => setIsNewGroupModalOpen(false)} title="TẠO NHÓM MỚI">
        ...
        <Button variant="primary" className="w-full mt-6" onClick={async () => {
          ...
          try {
            await api.saveVideoGroupTasks(newGroupDate, [{ day: 1, type: ExerciseType.MANDATORY, title: "Khởi động phác đồ", detail: "Bài tập mặc định", link: "", is_deleted: false }]);
      ...
      <Modal isOpen={isTaskModalOpen} onClose={() => setIsTaskModalOpen(false)} title={editingTask ? "SỬA BÀI TẬP MẪU" : "THÊM BÀI TẬP MẪU"} maxWidth="max-w-2xl">
        ...
        <Button variant="primary" className="w-full mt-6 py-4" onClick={async () => {
          ...
          try {
            ...
            const nextTasks = editingTask ? currentGroupTasks.map(x => x._rowNumber === editingTask.task._rowNumber ? (newTaskData as ExerciseTask) : x) : [...currentGroupTasks, newTaskData as ExerciseTask];
            await api.saveVideoGroupTasks(groupData.video_date, nextTasks);
            await fetchTasksForSelected(selectedKeys);
